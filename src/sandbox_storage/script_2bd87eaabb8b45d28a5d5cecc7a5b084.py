...
print("=== 按LOGTYPE统计访问量 ===")
print(logtype_stats.to_string(index=False))
print("\n" + "="*80 + "\n")
...
print("=== 按IP地址统计 ===")
print(ip_stats.to_string(index=False))
print("\n" + "="*80 + "\n")
...
print("\n=== 可视化数据准备 ===")
# 创建帕累托图数据
pareto_data = action_stats.copy()
pareto_data['累计访问次数'] = pareto_data['访问次数'].cumsum()
pareto_data['累计占比(%)'] = (pareto_data['累计访问次数'] / len(df) * 100).round(2)

print("帕累托分析数据:")
print(pareto_data[['ACTION', '访问次数', '占比(%)', '累计访问次数', '累计占比(%)']].to_string(index=False))
